...
class Environment(object):
    ...
    # DEFAULT_STATE_RANGE = np.concatenate([np.ones(2*len(ACTION_KEYS)) * 70, np.array([4.3])])
    ...
    def __init__(
        ...
        self.state_dim = len(self.ACTION_KEYS)*2  + 10
    ...
    def generate_reward(self, state, time, is_fallen):
        ...
        if is_fallen:
            ...
            # reward -= 1000
            # reward -= 1000 * np.exp(-(time-self.init_time)/self.SIMULATION_TIME)
            # reward -= 1500
        # print("TARGET:", target)
